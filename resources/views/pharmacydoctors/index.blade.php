...
<div class="d-flex align-content-stretch flex-wrap" style="text-align:center">
  ...
  <a href="{{route('pharmacydoctors.create')}}"  class="btn btn-success mb-5" style="align-center" >Add New Doctor</a></div>
      <table class="table table-bordered table-hover table-dark" class="mx-auto" style="background-color: 	rgb(52, 57, 64)" id="doctor_table">
        <thead class="thead-light">
          ...
          <tbody>
            ...
            <td><a href="{{route('pharmacydoctors.show',['doctor' => $doctor->id])}}" class="btn btn-primary btn-sm" >  <i class="fas fa-folder">
            </i> View</a></td>
              <td><a href="{{route('pharmacydoctors.edit',['doctor' => $doctor->id])}}" class="btn btn-info btn-sm" > <i class="fas fa-pencil-alt">
            </i> Edit</a></td>
            <td> 
                <form method="POST" action="{{route('pharmacydoctors.destroy',['doctor' => $doctor->id])}}" >
                    @method('DELETE')
                    @csrf
